...
- **Работает на LLM** (DeepSeek): генерация — `deepseek-flash`; старшая `deepseek-v4-pro`
  используется только для локальных проверок промптов (LLM-as-judge) и в рантайме не вызывается;
...
- **Логи**: тексты сообщений и ответов пишутся в лог (без них поведение не разобрать), плюс полный промпт и сырой ответ модели на уровне `debug`. На сервере логи ротируются: суточный дамп в `/var/log/memes-bot/bot.log`, `logrotate` хранит текущий файл и два прошлых дня, вывод контейнера ограничен по размеру;
