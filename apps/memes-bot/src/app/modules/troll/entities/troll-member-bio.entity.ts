...
export interface TrollMemberBioFact {
  ...
  weight: number;
  /**
   * Эмоциональная окраска факта: +1 позитив, 0 нейтрально, -1 негатив.
   * Опционально (у старых записей отсутствует — считается нейтральной).
   * Нужна для слоя «настроение ↔ память»: негативные воспоминания портят
   * настроение, а настроение смещает выборку воспоминаний.
   */
  valence?: number;
}

/** Одно заметное событие эпизодической памяти (что человек делал недавно). */
export interface TrollMemberBioEvent {
  /** Короткая формулировка события («проебал дедлайн», «сходил на шашлыки»). */
  text: string;
  /** Когда событие видели в последний раз (epoch ms) — для вымывания по давности. */
  seenAt: number;
}
...
export class TrollMemberBioEntity {
  ...
  /**
   * Заметные недавние события (эпизодическая память): что человек делал/о
   * чём рассказывал («проебал дедлайн», «сходил на шашлыки»). Живут короче
   * фактов — вымываются по давности.
   */
  @Column('jsonb', { nullable: true })
  events: TrollMemberBioEvent[] | null;

  /** Мнение бота о человеке (рефлексия): короткая оценка, обновляется. */
  @Column('varchar', { nullable: true })
  opinion: string | null;
