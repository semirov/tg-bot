...
export class AppService implements OnModuleInit {
  ...
  private async prepareCaptchaConversation(
    ...
    // grammY conversations v2 не всегда передаёт outside-ctx в external;
    // fallback — ctx вызова (сессия уже загружена session-middleware).
    const sessionCtx = (c: unknown): BotContext => (c as BotContext) ?? ctx;
    const captchaValues = await conversation.external((c) =>
      sessionCtx(c).session.captchaValues
    );
